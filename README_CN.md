...
- **系统信息**：快捷链接、版本检查、请求日志开关、本地登录信息清理，以及拉取 `/v1/models` 并分组展示（需要至少一个代理 API Key 才能查询模型）。
  ...
  更新通知弹窗仅在页面首次加载或浏览器刷新（F5 / Ctrl+R / 重新加载）时出现，不会因页面间的客户端 hash 路由切换而触发（例如从 `#/ai-providers` 切换到 `#/auth-files`）。当有可用更新时，弹窗会在当前会话中显示一次，直到用户手动关闭为止；在“系统信息”页面也提供手动“再次检查”的操作。
