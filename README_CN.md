...
- **系统信息**：快捷链接、版本检查、请求日志开关、本地登录信息清理，以及拉取 `/v1/models` 并分组展示（需要至少一个代理 API Key 才能查询模型）。
  ...
  自动更新检查只会在一次浏览器加载中的首次连接后运行，包括首次访问和浏览器刷新（F5 / Ctrl+R / 重新加载）。它不会因页面间的客户端 hash 路由切换而触发（例如从 `#/ai-providers` 切换到 `#/auth-files`）。当上游 fork 报告可用更新时，弹窗会显示当前连接服务的版本、可用版本/提交及 `CHANGELOG.md` 链接；在“系统信息”页面仍可手动“再次检查”。
