...
- **OAuth**: start OAuth/device flows for Codex, Anthropic/Claude, Antigravity, Kimi, and xAI/Grok; poll status; submit callback URLs or xAI/Grok displayed codes; import Vertex JSON credentials and iFlow cookies. Includes a **"Do Not Use Proxy"** checkbox (checked by default): when checked, OAuth logins connect directly, bypassing the configured proxy; unchecking uses the configured proxy. The choice is sent per login request (`no_proxy=true`), so toggling takes effect without restarting the server. **Fork-specific feature** — protected across upstream merges by item 8 of the parent repo's `docs/MERGE-PRESERVATION-fork-fixes.md`. Every OAuth process result — login success/failure, polling errors, callback validation, cancellation, and Vertex/iFlow import outcomes — is presented as a centered **result modal** (built on the shared `Modal` component) instead of a small transient toast; the modal stays open with the localized result title, icon, and message until the user dismisses it, and only the link-copy feedback still uses a toast. For the main login result the modal shows **only** the localized `*_oauth_status_success` / `*_oauth_status_error` message (e.g. “Authentication successful!” / “Authentication failed:”) — runtime error details and waiting status text are never appended; detailed errors remain on the provider card's inline status line. **Fork-specific feature** — protected across upstream merges by item 9 of the parent repo's `docs/MERGE-PRESERVATION-fork-fixes.md`.
